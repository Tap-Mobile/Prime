    ...
    override suspend fun refreshSession(session: Session): ApiResult<Session> {
        ...
        return when (result) {
            is ApiResult.Success -> {
                logger.d(LogTag.REFRESH_TOKEN.name, "new access token: ${result.value.accessToken.formatToken(client)}")
                logger.d(LogTag.REFRESH_TOKEN.name, "new refresh token: ${result.value.refreshToken.formatToken(client)}")
                ApiResult.Success(
                    session.refreshWith(
