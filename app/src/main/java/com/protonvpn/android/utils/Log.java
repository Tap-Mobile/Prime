...
public final class Log {
    ...
    public static void checkForLogTruncation(String filePath) {
        ...
        if (file.length() > Constants.MAX_LOG_SIZE) {
            try (FileChannel outChan = new FileOutputStream(file, true).getChannel()) {
                ...
                AppCrashlytics.INSTANCE.recordException(e, true);
            }
        }
    }
    ...
    private static String withSourceInfo(String msg) {
        PrimeLogger.INSTANCE.log(msg);
        final StackTraceElement trace = Thread.currentThread().getStackTrace()[4];
        return String.format(Locale.getDefault(), "(%1$s:%2$d) %3$s", trace.getFileName(), trace.getLineNumber(), msg);
    }
}