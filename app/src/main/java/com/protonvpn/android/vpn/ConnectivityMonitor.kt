...
class ConnectivityMonitor(
    ...
    private val networkCallback = object : ConnectivityManager.NetworkCallback() {
        ...
        override fun onLosing(network: Network, maxMsToLive: Int) {
            PrimeLogger.log("Loosing network ($maxMsToLive)")
        }

        override fun onUnavailable() {
            PrimeLogger.log("Network unavailable")
        }

        override fun onAvailable(network: Network) {
            PrimeLogger.log("Network available")
        }

        override fun onLost(network: Network) {
            PrimeLogger.log("Network lost")
        }
    }
    ...
    init {
        ...
        context.registerBroadcastReceiver(IntentFilter(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
            PrimeLogger.log("Airplane mode: " + it?.getBooleanExtra("state", false))
        }
    }
