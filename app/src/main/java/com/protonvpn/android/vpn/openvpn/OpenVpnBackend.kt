...
class OpenVpnBackend(
    ...
    init {
        ...
        VpnStatus.addLogListener {
            PrimeLogger.log(it.getString(PrimeApp.getAppContext()))
        }
    }
    ...
    private fun startOpenVPN(action: String?) {
        val ovpnService =
                Intent(PrimeApp.getAppContext(), OpenVPNWrapperService::class.java)
        if (action != null)
            ovpnService.action = action
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            PrimeApp.getAppContext().startForegroundService(ovpnService)
        } else {
            PrimeApp.getAppContext().startService(ovpnService)
        }
    }
