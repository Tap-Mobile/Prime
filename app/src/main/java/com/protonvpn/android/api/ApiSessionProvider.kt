...
//to do: true multi-user support will be added when core Auth is integrated.
class ApiSessionProvider : SessionProvider, SessionListener {
    ...
    private fun getLoginResponse(sessionId: SessionId) =
        currentSession?.takeIf { it.uid == sessionId.id }
            ?: oldSession?.takeIf { it.uid == sessionId.id }

    private fun getLoginResponse(userId: UserId) =
        currentSession?.takeIf { it.userId == userId.id }
            ?: oldSession?.takeIf { it.userId == userId.id }
    ...
    ) = SessionListener.HumanVerificationResult.Failure
